...
def createH5File(sid, subjList, sample_weights, predictions, fnRoot):
    ...
    sample_weights = divKernel_PSD[subjList.index(sid),:]    
    ...
    hdfPath = '/predicted'
    dset = f.create_dataset(hdfPath, predicted.shape, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sparseFile', type=str, help='input sparse graph file', required=False)
    parser.add_argument('--inputFile', type=str, help='input HDF5 file', required=False)
    ...
    if args['runtype'] == 0:
        ...
        knnGraphsRoot = args['inputFile']+sid
        subjList = list(f['/subjList'].value) # Used to find the index of the id
        ...
        # # Read the graph edge 
        # print( "working on :",sid)
        # EdgeMatrix_clean = loadSparseGraph(str(knnGraphsRoot) + '%04d'%subjList.index(sid)  + '.npz')
