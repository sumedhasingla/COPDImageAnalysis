...
class buildGraphs():
    ...
    def loadPickledData(self, useHarilick=False):     
    ...
    def buildSubjectTree(self, subj, data, neighbors=5):
        """
    ...
    def saveSubjectTree(self, trees, fn):
        """
...
def main():
    ...
    # Actually do things
    metaVoxelDict, subjList, phenotypeDB_clean, data = grapher.loadPickledData()
    # subjTree = buildSubjectTree(args.subject, subjList, data, args.neighbors)
    subjTree = Parallel(n_jobs=args.cores)(delayed(grapher.buildSubjectTree)(args.subject, data, args.neighbors) for i in xrange(1))
    fn = "/pylon1/ms4s88p/jms565/test_results/"+str(args.subject).zfill(4)
    # grapher.saveSubjectTree(subjTree[0], fn)
